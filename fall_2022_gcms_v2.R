# v2 started in june2023
# to do: remove unneccessary aromas (DONE, could remove another 3 perhaps)
...
col_med = apply(aroma_norm[8:36], 2, median) 

# see which to remove 
col_sd = apply(aroma_norm[8:36], 2, sd )
...
aroma_pca <- autoplot(pca_res, data = aroma_norm2,
                      colour = "treatment", # colours based on factor "community
  ...
  scale_colour_manual(values=c("grey", "chartreuse3", "orange"))+
  scale_fill_manual(values=c("grey", "chartreuse3","orange"))+
  # scale_colour_manual(values=c("grey", "violet", "deepskyblue", "coral2"))+ #colours for spatial
  # scale_fill_manual(values=c("grey", "violet", "deepskyblue", "coral2"))+ #colours for spatial
  facet_wrap("transfer") +
